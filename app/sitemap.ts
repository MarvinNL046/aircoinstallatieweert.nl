...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://aircoinstallatieweert.nl'
  ...
  // Generate city URLs with dynamic priorities based on population and focus on Weert
  const cityUrls = cities.map((city) => ({
    url: `${siteUrl}/steden/${city.slug}`,
    lastModified,
    changeFrequency: 'weekly' as const,
    priority: city.city.toLowerCase() === 'weert' ? 1.0 :
  ...
  // Generate service-city combination URLs with dynamic priorities and focus on Weert
  const serviceCityUrls = diensten.flatMap((dienst) =>
    cities.map((city) => ({
      url: `${siteUrl}/diensten/${dienst.slug}/${city.slug}`,
      lastModified,
      changeFrequency: 'weekly' as const,
      priority: city.city.toLowerCase() === 'weert' ? 0.9 :
